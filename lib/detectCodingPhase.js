...
// ── Layer 4: Explicit Code-Write Request ──
// Detects when the user is explicitly asking the copilot to WRITE CODE NOW,
// not just hint at an approach. These phrases override the bullet-hint prompt
// and route to the code-generation prompt instead.
const CODE_WRITE_TRIGGERS = [
  'write the code', 'write code', 'write it', 'write this',
  'code it', 'code this', 'code that',
  'implement it', 'implement this', 'implement that',
  'show me the code', 'show the code', 'show me how',
  'just write', 'go ahead', 'go ahead and write',
  'proceed', 'go proceed', 'go, proceed',
  'just do it', 'do it', 'do that',
  'let\'s go', "let's write", "let's code",
  'write a function', 'write a class', 'write a method',
  'give me the code', 'give me an implementation',
  'can you write', 'can you implement', 'can you code',
];

export function isCodeWriteRequest(transcript) {
  if (!transcript) return false;
  const lower = transcript.toLowerCase().trim();
  return CODE_WRITE_TRIGGERS.some(trigger => lower.includes(trigger));
}
 ...
 * @returns {{ active: boolean, reason: string, score: number, isCodeWriteRequest: boolean }}
 */
export function detectCodingPhase({ profilerState, transcript, clipboardCode, manualTerminalMode }) {
  // Layer 4: Explicit code-write request — detected independently of terminal mode
  const codeWrite = isCodeWriteRequest(transcript);

  // Layer 3: Manual override is absolute
  if (manualTerminalMode) {
    return { active: true, reason: 'manual_override', score: 100, isCodeWriteRequest: codeWrite };
  }

  // Layer 1: Profiler phase
  if (matchesProfilerPhase(profilerState)) {
    return { active: true, reason: 'profiler_phase', score: 80, isCodeWriteRequest: codeWrite };
  }
  ...
  if (transcriptScore >= TRANSCRIPT_THRESHOLD) {
    ...
    return {
      ...
      isCodeWriteRequest: codeWrite,
    };
  }

  // Layer 2b: Clipboard alone (weaker signal, needs at least SOME tech talk)
  if (hasClipboardCode(clipboardCode) && transcriptScore >= 2) {
    return { active: true, reason: 'clipboard+weak_transcript', score: 40, isCodeWriteRequest: codeWrite };
  }

  // Layer 4 alone: explicit write request even without other coding signals
  if (codeWrite) {
    return { active: true, reason: 'code_write_request', score: 70, isCodeWriteRequest: true };
  }

  return { active: false, reason: 'none', score: 0, isCodeWriteRequest: false };
}
