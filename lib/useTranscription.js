...
const DEBOUNCE_MS = 4000;          // Safety net — AssemblyAI's max_turn_silence (3.5s) does the heavy lifting
const COPILOT_REQUEUE_MS = 2000;   // Retry interval when copilot is in-flight
...
export function useTranscription(capabilities = {}, sessionContext = null) {
  ...
  const copilotFiringRef = useRef(false); // Lock: prevents parallel copilot fires
  const lastFiredQuestionRef = useRef(''); // Track last question for repeat detection
  ...
  const fireCopilot = useCallback(async (fullQuestion) => {
    copilotFiringRef.current = true; // Lock
    try {
      setActiveQuestion(fullQuestion); // Show the FULL accumulated question in the UI
      setPartialText('');              // Clear the accumulated preview
      setStatus('thinking');
      setBullets([]);
      setRawResponse('');
      ...
      setActiveQuestion(''); // Clear — history now has the full question
      setStatus('listening');
    } catch (copilotErr) {
      ...
      copilotFiringRef.current = false; // Unlock
    }
  }, []);

  // ── Debounce flush — runs after DEBOUNCE_MS of silence ──
  // AssemblyAI's max_turn_silence handles compound question merging natively.
  // This debounce is just a lightweight safety net + copilot lock.
  const flushDebounce = useCallback(() => {
    const fullQuestion = accumulatedTranscriptRef.current.trim();

    // Copilot lock: if one is already in-flight, re-queue and wait
    // This ensures the next fire sees the previous response in history (chain-of-thought)
    if (copilotFiringRef.current) {
      console.log('[debounce] Copilot in-flight — re-queuing, will retry in 2s');
      accumulatedTranscriptRef.current = fullQuestion; // put it back
      debounceTimerRef.current = setTimeout(flushDebounce, COPILOT_REQUEUE_MS);
      return;
    }

    accumulatedTranscriptRef.current = '';
    debounceTimerRef.current = null;
    setPartialText(''); // Clear the preview — text is now the copilot question

    if (!fullQuestion || !isStreamingRef.current) return;

    // ── Minimum word gate: skip filler like "Okay", "Alright", "Sure thing" ──
    ...
    if (shouldFire && isStreamingRef.current && capabilitiesRef.current.autoCopilot) {
      // Question repeat detection: if candidate is paraphrasing the last question, help them answer it
      const lastQ = lastFiredQuestionRef.current;
      let finalQuestion = fullQuestion;
      if (lastQ) {
        const overlap = computeWordOverlap(fullQuestion, [lastQ]);
        if (overlap > 0.5) {
          finalQuestion = `[CANDIDATE IS REPEATING/PARAPHRASING THE PREVIOUS QUESTION — help them ANSWER it, don't treat as new question]\nOriginal question: ${lastQ}\nCandidate said: ${fullQuestion}`;
          console.log(`[gate] REPEAT DETECTED (${(overlap * 100).toFixed(0)}% overlap) — augmenting context`);
        }
      }
      lastFiredQuestionRef.current = fullQuestion;
      fireCopilot(finalQuestion);
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      // Use dynamic prompt from session context, or static default
      ...
      ws.onopen = () => {
        // Send configure message with max_turn_silence FIRST (must be JSON message, not URL param)
        ws.send(JSON.stringify({ type: 'Configure', max_turn_silence: 3500, min_turn_silence: 300 }));
        console.log('[ws] Sent Configure: max_turn_silence=3500, min_turn_silence=300');
        ...
        try {
          ...
          if (msg.type === 'Begin') {
            ...
            if (endOfTurn && transcript.trim()) {
              ...
              // Show accumulated text as a preview so it doesn't vanish
              setPartialText(accumulatedTranscriptRef.current);
        ...
        if (attempt < MAX_RECONNECT) {
          ...
          reconnectTimerRef.current = setTimeout(async () => {
            try {
              ...
              const reconPrompt = sessionContextRef.current?.prompt || 'Technical job interview between two speakers. Speakers may pause mid-question.';
              const wsParams = new URLSearchParams({
                token: tokenData.token,
                sample_rate: '16000',
                speech_model: 'u3-rt-pro',
              ...
              wsRef.current = newWs;
              newWs.onopen = () => {
                // Send configure message on reconnect too
                newWs.send(JSON.stringify({ type: 'Configure', max_turn_silence: 3500, min_turn_silence: 300 }));
                console.log('[ws] Reconnect: Sent Configure: max_turn_silence=3500');
                setStatus('listening');
                setError(null);
