...
const DEBOUNCE_EXTEND_MS = 2000; // Extra wait when text looks like an incomplete sentence

// ── Sentence-completion detector ──
// Returns true if the accumulated text looks like the speaker is still mid-thought
function looksIncomplete(text) {
  if (!text) return false;
  const trimmed = text.trim();
  // Already has a question mark = complete question
  if (trimmed.endsWith('?')) return false;
  // Ends with a conjunction or comma = definitely continuing
  if (/\b(and|or|but|also|plus|as well as|what|how|where|when|why|which|if|whether|like)$/i.test(trimmed)) return true;
  if (trimmed.endsWith(',') || trimmed.endsWith('—') || trimmed.endsWith('-')) return true;
  // Very short fragment (< 5 words) that isn't a question = likely partial
  const wordCount = trimmed.split(/\s+/).length;
  if (wordCount < 5 && !trimmed.endsWith('.') && !trimmed.endsWith('!')) return true;
  return false;
}
...
export function useTranscription(capabilities = {}) {
  ...
  // Smart: if the accumulated text looks incomplete, extend the timer instead of flushing
  const flushDebounce = useCallback(() => {
    const fullQuestion = accumulatedTranscriptRef.current.trim();

    // Check if the sentence looks incomplete — extend debounce if so
    if (looksIncomplete(fullQuestion) && isStreamingRef.current) {
      console.log(`[debounce] Text looks incomplete, extending ${DEBOUNCE_EXTEND_MS}ms: "...${fullQuestion.slice(-40)}"`);
      debounceTimerRef.current = setTimeout(flushDebounce, DEBOUNCE_EXTEND_MS);
      return;
    }
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      const wsParams = new URLSearchParams({
        ...
        max_turn_silence: '2000',  // 2s silence before end_of_turn (default 1s) — reduces mid-question splits
      });
