...
export function useTranscription(capabilities = {}, sessionContext = null) {
  ...
  const start = useCallback(async () => {
    // Keep conversation history across stop/start; only reset transport/runtime state.
    setError(null);
    setIsPaused(false);
    setHeld(false);
  ...
  const pause = useCallback(() => {
    ...
    copilot.cancelInFlight();
    stopInternal();
    setIsPaused(true);
    setHeld(true);
    heldRef.current = true;
    setStatus('paused');
  }, [stopInternal, copilot]);
  ...
  const emergencyRescue = useCallback(async () => {
    ...
    heldRef.current = true;
    setStatus('thinking');
    await copilot.fire(rescueContext, 'candidate', { rescue: true, force: true });
    setStatus('paused');
  }, [copilot, gate.previewText, transcript, stopInternal]);
  ...
  useEffect(() => {
    ...
    brainFreezeTimerRef.current = setInterval(() => {
      // Use ref-based last speaker role (not React state — avoids stale closure)
      const lastRole = transcript.lastSpeakerRoleRef?.current;
      if (lastRole !== 'candidate') return; // Only detect freeze during candidate speech
