...
export function useCopilotFire({ capabilitiesRef, profilerStateRef, clipboardCodeRef, turnLogRef, lastPartialRef }) {
  ...
  const fire = useCallback(async (fullQuestion, speaker = 'interviewer', { rescue = false } = {}) => {
    ...
    try {
      ...
      if (codingDetection.isCodeWriteRequest) {
        console.log('[copilot] Code-write request detected — routing to code-gen prompt');
      }
      ...
      for await (const event of streamCopilot({
        ...
        clientTelemetry: {
          isRambling,
          isRescue: rescue,
          isCodeWriteRequest: codingPhaseRef.current.isCodeWriteRequest || false,
          topicAnchor: topicAnchorRef.current,
          recentTurns: buildTurnHistory().slice(-3),
        },
      })) {
