...
export function useCopilotFire({ capabilitiesRef, profilerStateRef, clipboardCodeRef }) {
  ...
  const fire = useCallback(async (fullQuestion, speaker = 'interviewer') => {
    ...
    try {
      ...
      for await (const event of streamCopilot({
        ...
        clientTelemetry: {
          isRambling: speakingStartRefBridge.current?.current && (Date.now() - speakingStartRefBridge.current.current) / 1000 >= 90,
        },
      })) {
