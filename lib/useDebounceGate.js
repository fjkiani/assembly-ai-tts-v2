...
export function useDebounceGate({ onFire, capabilitiesRef, copilotFiringRef, isStreamingRef, lastCopilotOutputRef }) {
  ...
  const flush = useCallback(() => {
    ...
    // ── CANDIDATE SUPPRESSION: Let Alpha talk! ──
    // Don't fire copilot when candidate is speaking. They need space to answer.
    // Only intervene after many consecutive candidate turns (they might be rambling).
    if (speaker === 'candidate') {
      candidateTurnCountRef.current += 1;
      if (candidateTurnCountRef.current < 6) {
        console.log(`[gate] CANDIDATE SPEAKING (turn ${candidateTurnCountRef.current}/6) — giving Alpha space. Suppressed.`);
        return;
      }
      console.log(`[gate] CANDIDATE RAMBLING (${candidateTurnCountRef.current} turns) — intervening with support.`);
    } else {
      // Interviewer is speaking — reset candidate turn counter
