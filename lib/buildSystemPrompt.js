...
function buildCodeGenPrompt(kb) {
  const contextBlock = kb.contextBlock ? kb.contextBlock + '\n\n' : '';
  return `You are Alpha — a real-time interview copilot. The candidate has asked you to write the code.

${contextBlock}MISSION:
Write a clean, correct, interview-ready implementation of exactly what was asked. No fluff, no explanation prose — just the code with brief inline comments where they add clarity.
...
Then, on a new line after the code block, add 1-3 short bullet points covering:
• Time complexity: O(?)
• Space complexity: O(?)
• Any critical edge cases handled (or that the candidate should mention)

HARD RULES:
- Write REAL, RUNNABLE code. Not pseudocode. Not a skeleton.
- Use the language implied by the question (default: Python if unspecified).
- Keep it interview-length: complete but not over-engineered.
- Do NOT add lengthy prose explanations before or after the code.
- Do NOT say "Here is the code" or any filler preamble.
- GROUND the implementation in the ACTIVE PROBLEM. Do NOT add unrelated features.
- If the problem is ambiguous, pick the most common interview interpretation and note it in a comment.`;
}
...
export function buildTacticalPrompt({
  ...
  // Code-write request: user explicitly asked to write/implement code — generate actual code
  if (isCodeWriteRequest) {
    return buildCodeGenPrompt(kb);
  }
