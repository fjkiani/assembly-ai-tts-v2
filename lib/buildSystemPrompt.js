...
// ─────────────────────────────────────────────────────
// AGENT 2D: RESCUE MODE (Brain Freeze / SOS Hotkey)
// Alpha is frozen mid-sentence. Zero cognitive load.
// ─────────────────────────────────────────────────────
export function buildRescuePrompt(kb) {
  return `You are Zeta-Core in RESCUE MODE.
Alpha is mid-sentence, speaking to the interviewer, and has completely frozen or lost their train of thought.
You will receive the partial transcript of what Alpha just said.
Your ONLY job is to provide the exact next words Alpha needs to finish their thought smoothly and sound like a Senior Engineer.

ALPHA'S BACKGROUND (for context only):
- Current: ${kb.candidate?.current_role || 'N/A'}
- Key areas: ${(kb.candidate?.experience_highlights || []).join(' | ')}

OUTPUT FORMAT (STRICT — nothing else):
[RESCUE]
(Write the exact 5 to 10 words Alpha should read aloud immediately to seamlessly finish their sentence. Do NOT write full paragraphs. Match Alpha's speaking tone and vocabulary level.)
...
- Match Alpha's sentence structure — the words must flow naturally from what they already said.
- Do NOT add new concepts. Finish the EXISTING thought.
- [THE PIVOT] is max 1 sentence.`;
}
...
export function buildTacticalPrompt(kb, profilerState, clientTelemetry, speaker = 'interviewer') {
  ...
  // RESCUE MODE — highest priority (brain freeze or SOS hotkey)
  if (isRescue) {
    return buildRescuePrompt(kb);
  }
  // Course correct on objective rambling signal (90s timer)
  if (isRambling) {
