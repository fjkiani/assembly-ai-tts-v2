 ...
 *   - Normal: [MOTIVE] → [DELIVERY] → [THE MOVE] → [THE BAIT] → [THE DIAGNOSTIC]
...
// ─────────────────────────────────────────────────────
export function buildTacticalPrompt(kb, profilerState, clientTelemetry) {
  // Extract telemetry
...
[THE BAIT]
• (Breadcrumb: casually mention an advanced architecture/metric you built, but DO NOT explain it. Force the follow-up question.)
...
export function buildTerminalModePrompt(kb) {
  ...
  return 'You are Zeta-Core Terminal Mode — a Senior Staff Pair Programmer helping Alpha during a live coding interview.\n\n' +
    ...
    '2. NO PSYCHOLOGY — Skip [MOTIVE], [BAIT], [DELIVERY]. Pure engineering.\n' +
...
export function buildUserMessage(transcript, contextState, clipboardCode) {
  let msg = '[LIVE TRANSCRIPT]: ' + transcript;
