...
export function useWebSocket({ capabilitiesRef, sessionContextRef, onMessage, onStatusChange, audioPipelineRef }) {
  ...
  const buildWsParams = useCallback((token) => {
    ...
    const wsParams = new URLSearchParams({
      ...
      speaker_labels: 'true',
      max_speakers: '2',  // Constrain to 2 speakers (interviewer + candidate) — improves label accuracy
      prompt: wsPrompt,
    });
    console.log('[ws] Speaker diarization ENABLED (max_speakers=2)');
  ...
  const sendConfigure = (wsInstance) => {
    // max_turn_silence: 3500ms — original working value. Spec default is 1000.
    //   3500 gives a generous pause without creating dead air.
    // min_turn_silence: 300ms — original working value. Spec default is 100.
    //   300 prevents cutting mid-word pauses.
    wsInstance.send(JSON.stringify({ type: 'UpdateConfiguration', max_turn_silence: 3500, min_turn_silence: 300 }));
    console.log('[ws] Sent UpdateConfiguration: max_turn_silence=3500, min_turn_silence=300');
  };
