...
export function useWebSocket({ capabilitiesRef, sessionContextRef, onMessage, onStatusChange, audioPipelineRef }) {
  ...
  const sendConfigure = (wsInstance) => {
    // max_turn_silence MUST be sent as JSON message, not URL param — URL param is silently ignored
    wsInstance.send(JSON.stringify({ type: 'Configure', max_turn_silence: 3500, min_turn_silence: 300 }));
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        ws.onclose = (closeEvent) => {
          ...
          const NON_RECOVERABLE = [1008, 4001, 4002];
